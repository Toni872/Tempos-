...
export class AutoClockService {
  ...
  private static async getUserScheduleForToday(
    userId: string,
    day: number,
  ...
  private static async ensureFicha(
    user: User,
    date: string,
    start: string,
    end: string,
    ...
    console.log(
      `📝 [AUTO-CLOCK] Creando ficha para ${user.email}: ${start} - ${end}`,
    );
    ...
    ficha.hoursWorked = parseFloat(
      ((h2 * 60 + m2 - (h1 * 60 + m1)) / 60).toFixed(2),
    );
