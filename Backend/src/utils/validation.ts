...
function getMaxObjectDepth(value: unknown, depth = 0): number {
  ...
  if (Array.isArray(value)) {
    return value.reduce<number>(
      (max, item) => Math.max(max, getMaxObjectDepth(item, depth + 1)),
      depth + 1,
    );
  }
  ...
  return objectValues.reduce<number>(
    (max, item) => Math.max(max, getMaxObjectDepth(item, depth + 1)),
    depth + 1,
  );
}
...
const fichaMetadataSchema = z
  .record(
    z.string().max(80, "metadata key admite maximo 80 caracteres."),
    z.unknown(),
  )
...
export const fichaStatusSchema = z.enum([
  "draft",
  "confirmed",
  "disputed",
  "archived",
...
export const createFichaSchema = z
  .object({
    ...
    description: z
      .string()
      .trim()
      .max(500, "description admite maximo 500 caracteres.")
      .optional(),
    projectCode: z
      .string()
      .trim()
      .max(80, "projectCode admite maximo 80 caracteres.")
      .optional(),
...
export const listFichasQuerySchema = z
  .object({
...
export const requestFichaCorrectionSchema = z
  .object({
    startTime: timeStringSchema.optional(),
    endTime: timeStringSchema.optional(),
    description: z
    ...
    reason: z
      .string()
      .trim()
      .min(10, "reason debe tener al menos 10 caracteres.")
      .max(500, "reason admite maximo 500 caracteres."),
  })
  .superRefine((value, ctx) => {
    const hasAnyChange =
...
export const reviewFichaCorrectionSchema = z.object({
  ...
  comment: z
    .string()
    .trim()
    .max(500, "comment admite maximo 500 caracteres.")
    .optional(),
});

export const closeFichaPeriodSchema = z
  .object({
