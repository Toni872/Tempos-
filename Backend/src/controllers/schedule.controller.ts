...
import { scheduleSchema, assignShiftSchema, buildValidationError } from '../utils/validation.js';
...
router.get('/', firebaseAuthMiddleware, appUserContextMiddleware, asyncHandler(async (req, res) => {
...
// Crear horario
router.post('/', firebaseAuthMiddleware, appUserContextMiddleware, asyncHandler(async (req, res) => {
  const auth = getAuthContext(req);
  if (auth.role !== 'admin' && auth.role !== 'manager') {
    res.status(403).json({ error: 'No tienes permisos para crear horarios.' });
    return;
  }
...
router.get('/assignments', firebaseAuthMiddleware, appUserContextMiddleware, asyncHandler(async (req, res) => {
  const auth = getAuthContext(req);
...
router.post('/assign', firebaseAuthMiddleware, appUserContextMiddleware, asyncHandler(async (req, res) => {
  const auth = getAuthContext(req);
  if (auth.role !== 'admin' && auth.role !== 'manager') {
    res.status(403).json({ error: 'No tienes permisos para asignar turnos.' });
    return;
  }
