...
test("isSameCompany returns false for different companies", () => {
  assert.equal(
    isSameCompany(ctx({ companyId: "tenant-a" }), "tenant-b"),
    false,
  );
});
...
test("isSameCompany returns false for suspended user even if company matches", () => {
  assert.equal(
    isSameCompany(
      ctx({ companyId: "tenant-a", status: "suspended" }),
      "tenant-a",
    ),
    false,
  );
...
test("admin from tenant-a cannot approve absence from tenant-b", () => {
  ...
  assert.equal(
    canAccessCompanyResource(auth, absenceFromTenantB, "approve_absence"),
    false,
  );
});
...
test("admin from same tenant can approve absence", () => {
  ...
  assert.equal(
    canAccessCompanyResource(auth, absence, "approve_absence"),
    true,
  );
});
...
test("manager from tenant-a cannot view employees from tenant-b", () => {
  ...
  assert.equal(
    canAccessCompanyResource(auth, employeeFromTenantB, "view_employees"),
    false,
  );
});
...
test("manager from same tenant can view employees", () => {
  ...
  assert.equal(
    canAccessCompanyResource(auth, employee, "view_employees"),
    true,
  );
});
...
test("manager from tenant-a cannot review ficha correction from tenant-b", () => {
  ...
  assert.equal(
    canAccessCompanyResource(auth, fichaFromTenantB, "review_ficha_correction"),
    false,
  );
});
...
test("manager from tenant-a cannot close period for tenant-b resources", () => {
  ...
  const fichaFromTenantB = { companyId: "tenant-b", ownerUserId: "user-22" };
  assert.equal(
    canAccessCompanyResource(auth, fichaFromTenantB, "close_ficha_period"),
    false,
  );
});
...
test("auditor from different company is blocked by isSameCompany guard", () => {
  ...
  assert.equal(
    canAccessCompanyResource(auth, resource, "view_employees"),
    false,
  );
});
...
test("deleted user loses all access to resources", () => {
  ...
  assert.equal(
    canAccessCompanyResource(auth, { companyId: "tenant-a" }),
    false,
  );
  assert.equal(canAccessOwnResource(auth, "user-1"), false);
});
...
testWithDb(
  "integration: fichas query scoping excludes rows from other tenants",
  async () => {
...
testWithDb(
  "integration: reports summary query includes only confirmed rows in same tenant",
  async () => {
    await ensureDbInitialized();
    ...
    try {
      ...
      const totalHours = confirmedRows.reduce(
        (acc, row) => acc + (Number(row.hoursWorked) || 0),
        0,
      );
...
testWithDb(
  "integration: absences query scoping excludes rows from other tenants",
  async () => {
    const { User } = await import("../entities/User.js");
...
testWithDb(
  "integration: documents query scoping excludes rows from other tenants",
  async () => {
    const { User } = await import("../entities/User.js");
...
testWithDb(
  "integration: audit-log query scoping keeps events inside tenant",
  async () => {
    await ensureDbInitialized();
...
testWithDb(
  "integration: audit-log filters by action and user inside tenant",
  async () => {
    await ensureDbInitialized();
