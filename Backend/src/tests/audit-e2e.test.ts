...
test('E2E Audit Simulation: Inspección de Trabajo LifeCycle', async () => {
  ...
  if (!dbUrl || dbUrl.includes('dummy')) {
    console.log('Skipping E2E audit integration test (require real DB). Set DATABASE_URL to PostgreSQL.');
    return;
  }
  ...
  assert.ok(clockInEntry.id, 'Debería haber un ID de evento CLOCK_IN inviolable');
  ...
  assert.ok(clockOutEntry.id, 'Debería haber un ID de evento CLOCK_OUT inviolable');

  // 3. Inspección exige el trazo atómico de esta Ficha:
  const inspectionEvents = await service.getsFichaEvents(ficha.id);
  assert.equal(inspectionEvents.length, 2, 'Inspección demanda 2 eventos atómicos para una jornada regular Web.');
  assert.equal(inspectionEvents[0].type, TimeEntryType.CLOCK_IN);
  assert.equal(inspectionEvents[1].type, TimeEntryType.CLOCK_OUT);
  assert.ok(inspectionEvents[0].latitude, 'La trazabilidad geolocalizada está presente e inyectada por el API del frontend.');
