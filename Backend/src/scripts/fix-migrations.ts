...
async function fix() {
  try {
    ...
    const history = [
      ...
      { timestamp: 1712600000000, name: 'AddUserCompanyIdAndRole1712600000000' },
      { timestamp: 1775756365116, name: 'SyncVarcharUids1775756365116' }
    ];

    for (const m of history) {
      const exists = await queryRunner.query('SELECT * FROM migrations WHERE name = $1', [m.name]);
      if (exists.length === 0) {
        await queryRunner.query('INSERT INTO migrations(timestamp, name) VALUES ($1, $2)', [m.timestamp, m.name]);
